...
import { Audit, AuditGenerateParams, AuditGenerateResponse } from './resources/audit';
import {
  ...
  CredentialRetrieveSchemaResponse,
...
import {
  SourceControl,
  SourceControlPullChangesParams,
  SourceControlPullChangesResponse,
} from './resources/source-control';
import {
  Tag,
  TagCreateParams,
  TagListParams,
...
import {
  DataTable,
  DataTableCreateParams,
  DataTableListParams,
  DataTableListResponse,
  DataTableUpdateParams,
  DataTables,
} from './resources/data-tables/data-tables';
import {
  Execution,
  ExecutionListParams,
...
import {
  ...
  WorkflowPublishParams,
  WorkflowRetrieveParams,
  WorkflowRetrieveVersionParams,
  WorkflowRetrieveVersionResponse,
...
export interface ClientOptions {
  /**
   * Defaults to process.env['N8N_MCP_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['N8N_MCP_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['N8N_MCP_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the N8n Mcp API.
 */
export class N8nMcp {
  apiKey: string;
   ...
   * API Client for interfacing with the N8n Mcp API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['N8N_MCP_API_KEY'] ?? undefined]
   * @param {string} [opts.baseURL=process.env['N8N_MCP_BASE_URL'] ?? https://api.example.com/api/v1] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('N8N_MCP_BASE_URL'),
    apiKey = readEnv('N8N_MCP_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.N8nMcpError(
        "The N8N_MCP_API_KEY environment variable is missing or empty; either provide it, or instantiate the N8nMcp client with an apiKey option, like new N8nMcp({ apiKey: 'My API Key' }).",
      );
    }
    ...
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://api.example.com/api/v1`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? N8nMcp.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('N8N_MCP_LOG'), "process.env['N8N_MCP_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('N8N_MCP_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://api.example.com/api/v1';
  }
  ...
  protected async authHeaders(opts: FinalRequestOptions): Promise<NullableHeaders | undefined> {
    return buildHeaders([{ 'X-N8N-API-KEY': this.apiKey }]);
  }
  ...
  static N8nMcpError = Errors.N8nMcpError;
   ...
   */
  sourceControl: API.SourceControl = new API.SourceControl(this);
  /**
   ...
   */
  dataTables: API.DataTables = new API.DataTables(this);
  /**
...
N8nMcp.Audit = Audit;
N8nMcp.Credentials = Credentials;
N8nMcp.Executions = Executions;
N8nMcp.Tags = Tags;
N8nMcp.Workflows = Workflows;
N8nMcp.Users = Users;
N8nMcp.SourceControl = SourceControl;
N8nMcp.Variables = Variables;
N8nMcp.DataTables = DataTables;
N8nMcp.Projects = Projects;

export declare namespace N8nMcp {
  ...
  export {
    SourceControl as SourceControl,
